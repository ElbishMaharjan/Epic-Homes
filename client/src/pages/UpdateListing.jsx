...
export default function CreatingListing() {
    ...
    const params = useParams();                     // Extract URL parameters from the current route using the useParams hook provided by React Route
    ...
    // useEffect hook to fetch listing data
    useEffect(() => {
        const fetchListing = async ()  => {   // Define a function to fetch listing data
            const listingId = params.listingId;  // Extract the listing ID from the params
            const res = await fetch(`/api/listing/get/${listingId}`);    // Fetch listing data from the server based on the listing ID
            const data =  await res.json();
            if (data.success === false) {  // If the response indicates failure, log the error message to the console and return
                console.log(data.message);
                return;
            }
            setFormData(data);    // Update the form data state with the fetched data
        };

        fetchListing();            // Call the fetchListing function when the component mounts or when 'params.listingId' changes
        
    }, [params.listingId]);       // Include 'params.listingId' in the dependency array to track its changes
...
const handleSubmit = async (e) =>{    // Define an asynchronous function to handle form submission
    ...
    try { 
        ...
        const res = await fetch(`/api/listing/update/${params.listingId}`, {   // Send a POST request to the '/api/listing/create' endpoint//we wana get response,use fetch method
